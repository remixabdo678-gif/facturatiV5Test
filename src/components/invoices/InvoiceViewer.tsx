...
export default function InvoiceViewer({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(
    user?.company?.defaultTemplate || 'template1'
  );
  ...
  // ---------- PDF ----------
  const generatePDFWithTemplate = async (): Promise<void> => {
    ...
    // Options html2pdf (tapées en any pour éviter les warnings TS sur pagebreak)
    const options: any = {
      margin: [5, 5, 5, 5],
      ...
      // Pas de "as const" ici -> évite l'erreur TSX
        ...
        <div className="inline-block w-full max-w-4xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            ...
            <div className="flex items-center space-x-3">
              ...
              {/* Impression (utilise la génération PDF pour cohérence) */}
              <button
                onClick={handlePrint}
